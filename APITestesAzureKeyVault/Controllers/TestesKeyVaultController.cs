...
namespace APITestesAzureKeyVault.Controllers;

[ApiController]
[Route("[controller]")]
public class TestesKeyVaultController : ControllerBase
{
    private readonly ILogger<TestesKeyVaultController> _logger;
    private readonly KeyClient _keyClient;

    public TestesKeyVaultController(ILogger<TestesKeyVaultController> logger,
    ...
    public async Task<Resultado> PostDecriptar(ParametrosOperacao dados)
    {
        ...
        _logger.LogInformation("Iniciando decriptação...");
        var decryptResult = await cryptoClient.DecryptAsync(
            EncryptionAlgorithm.RsaOaep,
            Convert.FromBase64String(dados.Conteudo!));
        _logger.LogInformation("Decriptação concluída...");
